...
const buscarDadosRastreadorPeloNome = (nome) => {
  return new Promise(async (resolve, reject) => {
    try {
      await axios
        .get(`${process.env.ORQUESTRADOR_API_BASE_URL}/v1/rastreadores/${nome}`, {
          timeout: 600000,
        })
        ...
        .catch((error) => {
          throw new Error(
            tratarMsgErroApi(
              error,
              "Ocorreu um erro inesperado ao 'buscarDadosRastreadorPeloNome'!"
            )
          );
        });
    } catch (error) {
...
const buscarIdUltimaMensagemRastreadorPeloIdEquipamentoRastreador = (
      ...
      await axios
        ...
        .catch((error) => {
          throw new Error(
            tratarMsgErroApi(
              error,
              "Ocorreu um erro inesperado ao 'vincularEquipamentoAoRastreador'!"
            )
          );
        });
    } catch (error) {
